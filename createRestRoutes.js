module.exports = class CreateRestRoutes {
  ...
  createRoutes(baseRoute, Model) {
    ...
    this.app.get(baseRoute + ".*", async (req, res) => {
      let query = decodeURIComponent(req.url.substr(req.url.indexOf("/.") + 1));
    ...
    this.app.get(baseRoute + ":id", async (req, res) => {
      let err,
        result = await Model.findById(req.params.id).catch(
          error => (err = error)
        );
      res.json(err || result);
    });
    ...
    this.app.delete(baseRoute + ":id", async (req, res) => {
      let err,
        result = await Model.findByIdAndRemove(req.params.id).catch(
          error => (err = error)
        );
      res.json(err || result);
    });
